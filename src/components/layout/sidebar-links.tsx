...
// ⚡ Bolt: Memoize individual static links to prevent re-rendering all of them on route change
...
export function SidebarLinks({ links }: SidebarLinksProps) {
    ...
    return (
        <>
        {links.map((link) => (
            <SidebarLinkItem key={link.href} link={link} isActive={pathname === link.href} />
        ))}
        </>
    )
}
