...
## 2024-05-15 - Stored DoS via Unvalidated Date Input
...
## 2026-05-24 - Prevent Rate Limit Bypass via Spoofable x-forwarded-for Header
**Vulnerability:** The application extracted the client IP for rate limiting using `headersList.get('x-forwarded-for')?.split(',')[0].trim()`. The first IP in the `x-forwarded-for` chain is client-controlled and easily spoofable, allowing attackers to completely bypass rate limit restrictions.
**Learning:** Never trust the first IP in the `x-forwarded-for` header for security enforcement. The last IP in the chain (`.pop()`) is appended by the application's trusted reverse proxy, making it the only reliable identifier.
**Prevention:** Always extract the last IP in the `x-forwarded-for` chain using `.split(',').pop()?.trim()` instead of `[0]`, or rely on a platform-specific verified IP header (like `x-real-ip`).
