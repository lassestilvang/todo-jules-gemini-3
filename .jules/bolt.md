...
## 2026-07-03 - Disabling Next.js prefetching in dense sidebars
...
## 2026-07-03 - Lazy Loading Dynamic Imports in Next.js
...
## 2026-07-03 - Localize usePathname() to prevent heavy parent re-renders
**Learning:** Calling `usePathname()` at the top level of a heavy parent component (like a Sidebar) forces the entire component tree (including expensive children like search overlays) to re-render on every client-side navigation.
**Action:** Extract navigation-dependent logic (like mapping active states on lists) into localized leaf components, pushing the `usePathname()` hook down so the parent component bypasses re-renders during route transitions.
