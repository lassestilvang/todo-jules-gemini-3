...
## 2024-07-01 - Prevent O(N) Re-Renders in dense Sidebar by pushing down usePathname
**Learning:** Calling `usePathname()` at the top level of a heavy parent component like a Sidebar forces the entire component tree to re-render on every client-side navigation.
**Action:** Extract navigation-dependent logic into localized leaf components, pushing the `usePathname()` hook down so the parent component bypasses re-renders during route transitions.
